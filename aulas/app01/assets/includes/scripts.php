...
<script
    src="<?php echo $comp ?>../vendor/plugins/tempusdominus-bootstrap-4/js/tempusdominus-bootstrap-4.min.js"></script>
<!-- Summernote -->
...
if (isset($_GET['msg']) && isset($_GET['status']) || $status = "fail") {
    $alert = TRUE;
    echo $status = isset($_GET['status']) ? $_GET['status'] : $status;
    echo $msg = isset($_GET['msg']) ? $_GET['msg'] : $msg;
    if ($status == 'success') {
        $icon = 'success';
        $title = 'Sucesso!';
    } else {
